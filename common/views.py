...
class NewsCategoryListApiView(APIView):
    ...
    def get(self, request):
        categories = models.NewsCategory.objects.values('id', 'name_uz', 'name_ru', 'name_en')
...
class PlayerManListApiView(APIView):
    ...
    def get(self, request):
        players = models.PlayerPosition.objects.prefetch_related(Prefetch('players', queryset=models.Players.objects.filter(gender=models.MAN)))
...
class PlayerWomanListApiView(APIView):
    @method_decorator(cache_page(2*60, key_prefix='player_list_cache'))
    def get(self, request):
        players = models.PlayerPosition.objects.prefetch_related(Prefetch('players', queryset=models.Players.objects.filter(gender=models.WOMEN)))
        serializer = serializers.PlayerPosiotionListSerializer(players, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class PlayerU19ListApiView(APIView):
    @method_decorator(cache_page(2*60, key_prefix='player_list_cache'))
    def get(self, request):
        players = models.PlayerPosition.objects.prefetch_related(Prefetch('players', queryset=models.Players.objects.filter(gender=models.U19)))
        serializer = serializers.PlayerPosiotionListSerializer(players, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class PlayerU21ListApiView(APIView):
    @method_decorator(cache_page(2*60, key_prefix='player_list_cache'))
    def get(self, request):
        players = models.PlayerPosition.objects.prefetch_related(Prefetch('players', queryset=models.Players.objects.filter(gender=models.U21)))
        serializer = serializers.PlayerPosiotionListSerializer(players, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class AboutAcademyApiView(ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        data = models.AboutAcademy.objects.all()
...
class CoachDetailApiView(RetrieveAPIView):
    ...
    queryset = models.Coach.objects.select_related('position')
    lookup_field = 'id'
...
class CoachTableListManApiView(ListAPIView):
    ...
    queryset = models.Coach.objects.filter(gender=models.MAN).only('id', 'full_name', 'position_id').select_related('position')
...
class CoachTableListWomenApiView(ListAPIView):
    serializer_class = serializers.CoachTableListSerializer
    queryset = models.Coach.objects.filter(gender=models.WOMEN).only('id', 'full_name', 'position_id').select_related('position')
...
class CoachAcademyListApiView(ListAPIView):
    serializer_class = serializers.CoachListSerializer
    queryset = models.Coach.objects.filter(coach_type=models.ACADEMY_COACH).select_related('position')
